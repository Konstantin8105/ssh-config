...
func TestGSSAPIClientIdentityInitFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapInit[GSSAPIClientIdentity]
	if !ok {
...
func TestGSSAPIClientIdentityValidFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapValid[GSSAPIClientIdentity]
	if !ok {
...
func TestGSSAPIClientIdentityParseFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapParse[GSSAPIClientIdentity]
	if !ok {
