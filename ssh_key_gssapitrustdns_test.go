...
func TestGSSAPITrustDnsInitFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapInit[GSSAPITrustDns]
	if !ok {
...
func TestGSSAPITrustDnsValidFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapValid[GSSAPITrustDns]
	if !ok {
...
func TestGSSAPITrustDnsParseFunc(t *testing.T) {
	defer func() {
		if err := recover(); err != nil {
			t.Errorf("panic in test is not acceptable")
		}
	}()
	f, ok := mapParse[GSSAPITrustDns]
	if !ok {
