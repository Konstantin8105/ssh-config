...
//      “keyword arguments”.  Configuration options may be separated by whitespace or optional white‐
//      space and exactly one ‘=’; the latter format is useful to avoid the need to quote whitespace
//      when specifying configuration options using the ssh, scp, and sftp -o option.  Arguments may
//      optionally be enclosed in double quotes (") in order to represent arguments containing spa‐
//      ces.
// 
...
//              only for those hosts that match one of the patterns given after the keyword.  If more
//              than one pattern is provided, they should be separated by whitespace.  A single ‘*’
//              as a pattern can be used to provide global defaults for all hosts.  The host is usu‐
//              ally the hostname argument given on the command line (see the CanonicalizeHostname
...
//              ditions are specified using one or more criteria or the single token all which always
//              matches.  The available criteria keywords are: canonical, exec, host, originalhost,
//              user, and localuser.  The all criteria must appear alone or immediately after
//              canonical.  Other criteria may be combined arbitrarily.  All criteria but all and
//              canonical require an argument.  Criteria may be negated by prepending an exclamation
//              mark (‘!’).
// 
...
//              useful to specify conditions that work with canonical host names only.  The exec key‐
//              word executes the specified command under the user's shell.  If the command returns a
//              zero exit status then the condition is considered true.  Commands containing white‐
//              space characters must be quoted.  The following character sequences in the command
//              will be expanded prior to execution: ‘%L’ will be substituted by the first component
//              of the local host name, ‘%l’ will be substituted by the local host name (including
//              any domain name), ‘%h’ will be substituted by the target host name, ‘%n’ will be sub‐
//              stituted by the original target host name specified on the command-line, ‘%p’ the
//              destination port, ‘%r’ by the remote login username, and ‘%u’ by the username of the
//              user running ssh(1).
// 
...
//              teria for the host keyword are matched against the target hostname, after any substi‐
//              tution by the Hostname or CanonicalizeHostname options.  The originalhost keyword
//              matches against the hostname as it was specified on the command-line.  The user key‐
//              word matches against the target username on the remote host.  The localuser keyword
//              matches against the name of the local user running ssh(1) (this keyword may be useful
...
//              passphrase are added to the agent with the default lifetime, as if by ssh-add(1).  If
//              this option is set to “ask”, ssh will require confirmation using the SSH_ASKPASS pro‐
//              gram before adding a key (see ssh-add(1) for details).  If this option is set to
...
//              tion.  Only useful on systems with more than one address.  Note that this option does
...
//              lookups.  If set to “yes” then, for connections that do not use a ProxyCommand,
//              ssh(1) will attempt to canonicalize the hostname specified on the command line using
...
//              new target name to pick up any new configuration in matching Host and Match stanzas.
// 
//      CanonicalizeMaxDots
...
//              the following escape characters: ‘%d’ (local user's home directory), ‘%u’ (local user
...
//              process, regardless of the setting of StrictHostKeyChecking.  If the option is set to
...
//              ssh(1) client for interoperability with legacy protocol 1 implementations that do not
//              support the 3des cipher.  Its use is strongly discouraged due to cryptographic weak‐
//              nesses.  The default is “3des”.
// 
//      Ciphers
//              Specifies the ciphers allowed for protocol version 2 in order of preference.  Multi‐
//              ple ciphers must be comma-separated.  If the specified value begins with a ‘+’ char‐
//              acter, then the specified ciphers will be appended to the default set instead of
//              replacing them.
// 
...
// 
//              The list of available ciphers may also be obtained using the -Q option of ssh(1) with
//              an argument of “cipher”.
// 
//      ClearAllForwardings
...
//              files, and is automatically set by scp(1) and sftp(1).  The argument must be “yes” or
...
//              be an integer from 1 (fast) to 9 (slow, best).  The default level is 6, which is good
//              for most applications.  The meaning of the values is the same as in gzip(1).  Note
...
//              using the default system TCP timeout.  This value is used only when the target is
...
//              will fall back to connecting normally if the control socket does not exist, or is not
//              listening.
// 
...
//              ever the display and agent forwarded will be the one belonging to the master connec‐
//              tion i.e. it is not possible to forward multiple displays or agents.
// 
//              Two additional options allow for opportunistic multiplexing: try to use a master con‐
//              nection but fall back to creating a new one if one does not already exist.  These
...
//              will be substituted by the target host name, ‘%n’ will be substituted by the original
//              target host name specified on the command line, ‘%p’ the destination port, ‘%r’ by
//              the remote login username, ‘%u’ by the username and ‘%i’ by the numeric user ID (uid)
//              of the user running ssh(1), and ‘%C’ by a hash of the concatenation: %l%h%p%r.  It is
//              recommended that any ControlPath used for opportunistic connection sharing include at
//              least %h, %p, and %r (or alternatively %C) and be placed in a directory that is not
//              writable by other users.  This ensures that shared connections are uniquely identi‐
//              fied.
// 
//      ControlPersist
...
//              nection will not be placed into the background, and will close as soon as the initial
//              client connection is closed.  If set to “yes” or “0”, then the master connection will
//              remain in the background indefinitely (until killed or closed via a mechanism such as
//              the ssh(1) “-O exit” option).  If set to a time in seconds, or a time in any of the
...
//              remote machine.
// 
//              The argument must be [bind_address:]port.  IPv6 addresses can be specified by enclos‐
//              ing addresses in square brackets.  By default, the local port is bound in accordance
//              with the GatewayPorts setting.  However, an explicit bind_address may be used to bind
//              the connection to a specific address.  The bind_address of “localhost” indicates that
...
//              This option should be placed in the non-hostspecific section.  See ssh-keysign(8) for
//              more information.
// 
//      EscapeChar
...
//              ter, or “none” to disable the escape character entirely (making the connection trans‐
//              parent for binary data).
// 
//      ExitOnForwardFailure
//              Specifies whether ssh(1) should terminate the connection if it cannot set up all
//              requested dynamic, tunnel, local, and remote port forwardings, (e.g. if either end is
//              unable to bind and listen on a specified port).  Note that ExitOnForwardFailure does
//              not apply to connections made over port forwardings and will not, for example, cause
//              ssh(1) to exit if TCP connections to the ultimate forwarding destination fail.  The
...
//              FORMATS section of sshd_config(5).  X11 connections received by ssh(1) after this
//              time will be refused.  The default is to disable untrusted X11 forwarding after
//              twenty minutes has elapsed.
// 
//      ForwardX11Trusted
//              If this option is set to “yes”, remote X11 clients will have full access to the orig‐
//              inal X11 display.
// 
...
//              the server. The default is unset, which means that the default identity will be used.
// 
...
//              If set to “yes” then renewal of the client's GSSAPI credentials will force the rekey‐
//              ing of the ssh connection. With a compatible server, this can delegate the renewed
//              credentials to a session on the server.  The default is “no”.
// 
//      GSSAPITrustDns
//              Set to “yes” to indicate that the DNS is trusted to securely canonicalize the name of
//              the host being connected to. If “no”, the hostname entered on the command line will
//              be passed untouched to the GSSAPI library.  The default is “no”.
// 
//      HashKnownHosts
...
//              separated pattern list.  Alternately if the specified value begins with a ‘+’ charac‐
//              ter, then the specified key types will be appended to the default set instead of
...
// 
//      HostKeyAlgorithms
//              Specifies the host key algorithms that the client wants to use in order of prefer‐
//              ence.  Alternately if the specified value begins with a ‘+’ character, then the spec‐
//              ified key types will be appended to the default set instead of replacing them.  The
...
//              replaced by a single ‘%’ character, which may be used when specifying IPv6 link-local
//              addresses.
// 
...
//              ~/.ssh/id_dsa, ~/.ssh/id_ecdsa, ~/.ssh/id_ed25519 and ~/.ssh/id_rsa for protocol ver‐
//              sion 2.  Additionally, any identities represented by the authentication agent will be
//              used for authentication unless IdentitiesOnly is set.  If no certificates have been
//              explicitly specified by CertificateFile, ssh(1) will try to load certificate informa‐
//              tion from the filename obtained by appending -cert.pub to the path of a specified
//              IdentityFile.
// 
//              The file name may use the tilde syntax to refer to a user's home directory or one of
//              the following escape characters: ‘%d’ (local user's home directory), ‘%u’ (local user
...
//              ties in an agent are offered during authentication.  IdentityFile may also be used in
//              conjunction with CertificateFile in order to provide any certificate also needed for
...
//              ple method names must be comma-separated.  The default is to use the server specified
//              list.  The methods available vary depending on what the server supports.  For an
...
//              can be specified by enclosing addresses in square brackets.  Multiple forwardings may
//              be specified, and additional forwardings can be given on the command line.  Only the
//              superuser can forward privileged ports.  By default, the local port is bound in
...
// 
//      LogLevel
//              Gives the verbosity level that is used when logging messages from ssh(1).  The possi‐
...
//              comma-separated.  If the specified value begins with a ‘+’ character, then the speci‐
//              fied algorithms will be appended to the default set instead of replacing them.
// 
...
//              case localhost will refer to a different machine on each of the machines and the user
//              will get many warnings about changed host keys.  However, this option disables host
//              authentication for localhost.  The argument to this keyword must be “yes” or “no”.
...
//              escape sequence in ssh(1).  The argument must be “yes” or “no”.  The default is “no”.
// 
//      PKCS11Provider
//              Specifies which PKCS#11 provider to use.  The argument to this keyword is the PKCS#11
//              shared library ssh(1) should use to communicate with a PKCS#11 token providing the
...
//              this option is set to “2,1” ssh will try version 2 and fall back to version 1 if ver‐
//              sion 2 is not available.  The default is ‘2’.  Protocol 1 suffers from a number of
//              cryptographic weaknesses and should not be used.  It is only offered to support
//              legacy devices.
// 
//      ProxyCommand
//              Specifies the command to use to connect to the server.  The command string extends to
//              the end of the line, and is executed using the user's shell ‘exec’ directive to avoid
//              a lingering shell process.
// 
//              In the command string, any occurrence of ‘%h’ will be substituted by the host name to
...
//              output.  It should eventually connect an sshd(8) server running on some machine, or
//              execute sshd -i somewhere.  Host key management will be done using the HostName of
//              the host being connected (defaulting to the name typed by the user).  Setting the
//              command to “none” disables this option entirely.  Note that CheckHostIP is not avail‐
...
//              separated pattern list.  Alternately if the specified value begins with a ‘+’ charac‐
//              ter, then the key types after it will be appended to the default instead of replacing
//              it.  The default for this option is:
// 
//                 ecdsa-sha2-nistp256-cert-v01@openssh.com,
//                 ecdsa-sha2-nistp384-cert-v01@openssh.com,
//                 ecdsa-sha2-nistp521-cert-v01@openssh.com,
...
//              optional second value is specified in seconds and may use any of the units documented
//              in the TIME FORMATS section of sshd_config(5).  The default value for RekeyLimit is
...
//              [bind_address:]port and the second argument must be host:hostport.  IPv6 addresses
//              can be specified by enclosing addresses in square brackets.  Multiple forwardings may
//              be specified, and additional forwardings can be given on the command line.  Privi‐
//              leged ports can be forwarded only when logging in as root on the remote machine.
// 
...
//              exists, or an authentication agent is running.  The default is “yes”.  Note that this
//              option applies to protocol version 1 only.
// 
//      SendEnv
//              Specifies what variables from the local environ(7) should be sent to the server.  The
//              server must also support it, and the server must be configured to accept these envi‐
//              ronment variables.  Note that the TERM environment variable is always sent whenever a
//              pseudo-terminal is requested as it is required by the protocol.  Refer to AcceptEnv
//              in sshd_config(5) for how to configure the server.  Variables are specified by name,
//              which may contain wildcard characters.  Multiple environment variables may be sepa‐
//              rated by whitespace or spread across multiple SendEnv directives.  The default is not
...
//              be sent to the server, or 300 if the BatchMode option is set.  ProtocolKeepAlives and
...
//              all new hosts.  If this flag is set to “no”, ssh will automatically add new host keys
//              to the user known hosts files.  If this flag is set to “ask”, new host keys will be
//              added to the user known host files only after the user has confirmed that is what
...
//              level keepalives), so takes a long time to notice when the connection dies.  As such,
//              you probably want the ServerAliveInterval option as well.  However, this means that
//              connections will die if the route is down temporarily, and some people find it annoy‐
//              ing.
// 
...
//              “yes” requests the default tunnel mode, which is “point-to-point”.  The default is
//              “no”.
// 
//      TunnelDevice
...
//              (remote_tun).
// 
//              The argument must be local_tun[:remote_tun].  The devices may be specified by numeri‐
//              cal ID or the keyword “any”, which uses the next available tunnel device.  If
...
//              learning alternate hostkeys for a server and supports graceful key rotation by allow‐
//              ing a server to send replacement public keys before old ones are removed.  Additional
//              hostkeys are only accepted if the key used to authenticate the host was already
//              trusted or explicitly accepted by the user.  If UpdateHostKeys is set to “ask”, then
//              the user is asked to confirm the modifications to the known_hosts file.  Confirmation
//              is currently incompatible with ControlPersist, and will be disabled if it is enabled.
// 
...
//              used on different machines.  This saves the trouble of having to remember to give the
...
//              secure fingerprint from DNS.  Insecure fingerprints will be handled as if this option
//              was set to “ask”.  If this option is set to “ask”, information on fingerprint match
//              will be displayed, but the user will still need to confirm new host keys according to
//              the StrictHostKeyChecking option.  The argument must be “yes”, “no”, or “ask”.  The
//              default is “no”.
// 
//              See also VERIFYING HOST KEYS in ssh(1).
// 
//      VisualHostKey
//              If this flag is set to “yes”, an ASCII art representation of the remote host key fin‐
//              gerprint is printed in addition to the fingerprint string at login and for unknown
//              host keys.  If this flag is set to “no”, no fingerprint strings are printed at login
//              and only the fingerprint string will be printed for unknown host keys.  The default
//              is “no”.
// 
//      XAuthLocation
